...
<p>меняем строки:</p>
<p>subnet ${e.hqRtr.interfaces.hqSrv.netAddress} netmask ${be(e.hqRtr.interfaces.hqSrv.mask)} {</p>
...
<p>0	IN	SOA	${w(e.hqRtr.interfaces.hqCli.netAddress)}.in-addr.apra. root.${w(e.hqRtr.interfaces.hqCli.netAddress)}.in-addr.apra. (...)</p>
<p>	IN	NS	${w(e.hqRtr.interfaces.hqCli.netAddress)}.in-addr.apra.</p>
...
<p>Убираем лишнее и пишем основные моменты:</p>
<p>0	IN	SOA	${w(e.hqRtr.interfaces.hqSrv.netAddress)}.in-addr.apra. root.${w(e.hqRtr.interfaces.hqSrv.netAddress)}.in-addr.apra. (...)</p>
<p>	IN	NS	${w(e.hqRtr.interfaces.hqSrv.netAddress)}.in-addr.apra.</p>
<p>	IN	A	${e.hqSrv.interfaces.hqRtr.ip}</p>
...
`,we=(e,{devices:t})=>{const n=document.createElement("table"),r=document.createElement("thead");r.innerHTML=`
  ...
  `,n.appendChild(r);const s=document.createElement("tbody");Object.values(t).forEach(({name:c,interfaces:a})=>{const d=Object.values(a);if(!d.length)return;const f=document.createElement("tr");f.innerHTML=`
    ...
    `,s.appendChild(f);for(let S=1;S<d.length;S+=1){const u=d[S],$=document.createElement("tr");$.innerHTML=`
        ...
        <td>${u.mask}</td>
        <td>${u.gateway}</td>
      `,s.appendChild($)}}),n.appendChild(s),e.ipTableContainer.innerHTML="",e.ipTableContainer.appendChild(n)},ge=(e,t)=>{const n=qe(t);e.textContainer.innerHTML=n,document.querySelectorAll("code").forEach(r=>{const s=document.createElement("span");s.className="code-wrapper";const c=document.createElement("div");c.className="code-copy-toast",c.textContent="Скопировано!",r.parentNode.insertBefore(s,r),s.appendChild(r),s.appendChild(c),r.style.cursor="pointer",r.title="Кликни, чтобы скопировать",r.addEventListener("click",()=>{navigator.clipboard.writeText(r.innerText).then(()=>{c.classList.add("show"),setTimeout(()=>c.classList.remove("show"),300)})})})},ye=(e,t)=>(n,r)=>{switch(n){case"devices":we(e,t),ge(e,t);break}};function $e(e){const t=new Set;return Object.values(e).forEach(n=>{Object.values(n.interfaces).forEach(r=>{r.netAddress&&!r.netAddress.startsWith("<")&&t.add(r.netAddress)})}),Array.from(t)}function Ae(e,t){const n=document.createElement("form");n.id="network-form",n.innerHTML="<h3>Настройки сетей</h3>",e.forEach(s=>{const c=document.createElement("div");c.className="network-input";const a=document.createElement("label");a.textContent=`Сеть ${s}:`;const d=document.createElement("input");d.type="text",d.placeholder="Базовый адрес (напр. 192.168.0)",d.dataset.network=s;const f=document.createElement("input");f.type="text",f.placeholder="Маска (напр. 24)",f.dataset.network=s,c.append(a,d,f),n.append(c)});const r=document.createElement("button");r.type="button",r.textContent="Обновить",n.append(r),t.innerHTML="",t.append(n)}function ke(){const e=document.querySelectorAll("#network-form input"),t={};return e.forEach(n=>{const{network:r}=n.dataset;t[r]||(t[r]={}),n.placeholder.includes("Базовый")?t[r].base=n.value.trim():t[r].mask=n.value.trim()}),t}function _e(e,t){const n=JSON.parse(JSON.stringify(e));return Object.entries(t).forEach(([r,{base:s,mask:c}])=>{!s||!c||Object.values(n).forEach(a=>{Object.values(a.interfaces).forEach(d=>{if(d.netAddress===r){d.mask=c;const[f]=d.ip.split(".").slice(-1);if(d.ip=`${s}.${f}`,d.netAddress=`${s}.0`,d.gateway!=="-"){const S=d.gateway.split(".");S.splice(0,3,...s.split(".")),d.gateway=S.join(".")}}})})}),n}const G={isp:{name:"ISP",interfaces:{external:{name:"ens18",ip:"<ВНЕШНИЙ>",mask:"24",gateway:"<ВНЕШНИЙ>",destination:"Интернет",netAddress:""},hqRtr:{name:"ens19",ip:"172.16.4.1",mask:"28",gateway:"-",destination:"HQ-RTR",netAddress:"172.16.4.0"},brRtr:{name:"ens20",ip:"172.16.5.1",mask:"28",gateway:"-",destination:"BR-RTR",netAddress:"172.16.5.0"}}},hqRtr:{name:"HQ-RTR",interfaces:{isp:{name:"ens18",ip:"172.16.4.2",mask:"28",gateway:"172.16.4.1",destination:"ISP",netAddress:"172.16.4.0"},brRtr:{name:"gre1",ip:"10.5.5.1",mask:"30",gateway:"-",destination:"BR-RTR",netAddress:"10.5.5.0"},hqCli:{name:"VLAN100",ip:"192.168.100.1",mask:"28",gateway:"-",destination:"HQ-CLI",netAddress:"192.168.100.0"},hqSrv:{name:"VLAN200",ip:"192.168.200.1",mask:"28",gateway:"-",destination:"HQ-SRV",netAddress:"192.168.200.0"},vlan999:{name:"VLAN999",ip:"192.168.99.1",mask:"29",gateway:"-",destination:"VLAN999 (?)",netAddress:"192.168.99.0"}}},hqSrv:{name:"HQ-SRV",interfaces:{hqRtr:{name:"ens18",ip:"192.168.100.2",mask:"28",gateway:"192.168.100.1",destination:"HQ-RTR",netAddress:"192.168.100.0"}}},hqCli:{name:"HQ-CLI",interfaces:{hqRtr:{name:"ens18",ip:"192.168.200.2",mask:"28",gateway:"192.168.200.1",destination:"HQ-RTR",netAddress:"192.168.200.0"}}},brRtr:{name:"BR-RTR",interfaces:{isp:{name:"ens18",ip:"172.16.5.2",mask:"28",gateway:"172.16.5.1",destination:"ISP",netAddress:"172.16.5.0"},brSrv:{name:"ens19",ip:"192.168.0.1",mask:"28",gateway:"-",destination:"BR-SRV",netAddress:"192.168.0.0"},hqRtr:{name:"gre1",ip:"10.5.5.2",mask:"30",gateway:"-",destination:"HQ-RTR",netAddress:"10.5.5.0"}}},brSrv:{name:"BR-SRV",interfaces:{brRtr:{name:"ens18",ip:"192.168.0.2",mask:"28",gateway:"192.168.0.1",destination:"BR-RTR",netAddress:"192.168.0.0"}}}},Ce=()=>{const e={ipTableContainer:document.getElementById("ip-table-container"),textContainer:document.getElementById("text-container"),networkFormContainer:document.getElementById("network-form-container")},t={devices:[]},n=N(t,ye(e,t));n.devices=G;const r=JSON.parse(JSON.stringify(G)),s=$e(r);Ae(s,e.networkFormContainer),document.querySelector("#network-form button").addEventListener("click",()=>{const c=ke(),a=_e(r,c);console.log(a),n.devices=a})};Ce();
